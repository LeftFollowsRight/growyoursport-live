	...
	if (typeof wp !== 'undefined' && wp.media && wp.media.view.AttachmentFilters) {
		...
		wp.media.view.AttachmentsBrowser = wp.media.view.AttachmentsBrowser.extend({
			createToolbar: function() {
				...
				var categoryLabel = new wp.media.view.Label({
					value: SiteParameters.media_cats.filter_label || 'Filter by category',
					attributes: {
						'for': 'media-category-filters'
					},
					priority: -80
				});

				var categoryFilter = new MediaLibraryTaxonomyFilter({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				});

				// Add the label + select straight to the toolbar (no wrapper div).
				this.toolbar.set( 'MediaLibraryTaxonomyFilter-label', categoryLabel.render() );
				this.toolbar.set( 'MediaLibraryTaxonomyFilter', categoryFilter.render() );
			}
		});
