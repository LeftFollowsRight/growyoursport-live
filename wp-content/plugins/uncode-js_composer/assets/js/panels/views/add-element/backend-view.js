		...
		.extend({
			...
			buildFiltering: function () {
				...
				if ( false !== tag && !_.isUndefined( vc.getMapped( tag ).allowed_container_element ) ) {
					...
					} else if ( _.isString( vc.getMapped( tag ).allowed_container_element ) || _.isArray( vc.getMapped( tag ).allowed_container_element ) ) {
						var allowedContainers = vc.getMapped( tag ).allowed_container_element;
						if ( _.isString( allowedContainers ) ) {
							allowedContainers = allowedContainers.split( ',' );
						}
						var allowedSelector = _.map( allowedContainers, function( el ) {
							return '[data-element=' + el.trim() + ']';
						} ).join( ',' );
						itemSelector += ':not([data-is-container=true]), ' + allowedSelector;
					}
					// END UNCODE EDIT
				}
