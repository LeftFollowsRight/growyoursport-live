(function($) {
	...
	UNCODE.frontendEditorSidebarSwitch = function(){
		...
		var $navBar = $('#vc_navbar', parent.document),
			...
			$cstm_code = $('> .vc_pull-right', $navBarUl).has('#vc_custom-code-button').addClass('has_vc_custom-code-button'),
	...
	if ( typeof window.parent.vc !== 'undefined' ) {
		...
		window.parent.vc.events.on( "vc.update-slider", function(model) {
			...
			if ($carousel && typeof $carousel.settings !== 'undefined') {
		...
		window.parent.vc.events.on('shortcodeView:ready', function(model) {
			...
			if ( shortcode === 'uncode_menu_block' ) {
				UNCODE.unBlockMenu();
			}
	...
	(function() {
		...
		function pasteSettings(model, $btn) {
			...
			var targetShortcode = model.get('shortcode');
			var sourceShortcode = stored.shortcode;
			var currentParams = model.get('params') || {};
			var newParams;
		...
		function updateAllPasteButtons() {
			var stored = getStoredSettings();
			$('.vc_control-btn-paste-settings').each(function() {
				var $btn = $(this);
				if (!stored) {
		...
		// Helper function to inject buttons into a single controls area
		function injectButtonsIntoControls($controls, model, insertBefore) {
			var $referenceBtn = $controls.find('.vc_control-btn-delete').first();
			if (!$referenceBtn.length) return;

			// Check if already injected
			if ($controls.find('.vc_control-btn-copy-settings').length) return;

			var stored = getStoredSettings();
			var pasteDisabled = !stored ? ' vc_btn-disabled' : '';

			var $copyBtn = $('<a class="vc_control-btn vc_control-btn-copy-settings" href="#" title="Copy Settings"><span class="vc_btn-content">C<span>S</span></span></a>');
			var $pasteBtn = $('<a class="vc_control-btn vc_control-btn-paste-settings' + pasteDisabled + '" href="#" title="Paste Settings"><span class="vc_btn-content">P<span>S</span></span></a>');

			if (insertBefore) {
				// For rows: insert before delete button (special positioning)
				$copyBtn.insertBefore($referenceBtn);
				$pasteBtn.insertBefore($referenceBtn);
			} else {
				// For other elements: insert after delete button
		...
		function injectSettingsButtons($el, model) {
			var shortcode = model.get('shortcode');
			var isSection = shortcode === 'vc_section';
			var $controls;

			if (shortcode === 'vc_row') {
				// For rows: inject into ALL .parent-vc_row .vc_advanced (one per column)
				$el.find('.parent-vc_row .vc_advanced').each(function() {
					injectButtonsIntoControls($(this), model, false);
				});
				return;
			} else if (shortcode === 'vc_row_inner') {
				// For row_inner: inject into ALL .parent-vc_row_inner .vc_advanced (one per column)
				var $innerControls = $el.find('.parent-vc_row_inner .vc_advanced');
				if (!$innerControls.length) {
					$innerControls = $el.siblings('.vc_controls-column').find('.parent-vc_row_inner .vc_advanced');
				}
				if (!$innerControls.length) {
					$innerControls = $el.closest('.vc_vc_row_inner').find('.parent-vc_row_inner .vc_advanced');
				}
				$innerControls.each(function() {
					injectButtonsIntoControls($(this), model, false);
				});
				return;
			} else if (shortcode === 'vc_column') {
				// For columns: .element-vc_column .vc_advanced
				$controls = $el.find('.element-vc_column .vc_advanced').first();
				// Also inject parent row CS/PS into this column's row controls
				var $rowControls = $el.find('.parent-vc_row .vc_advanced').first();
				if ($rowControls.length) {
					var parentId = model.get('parent_id');
					var parentModel = parentId && window.parent.vc.shortcodes ? window.parent.vc.shortcodes.get(parentId) : null;
					if (parentModel) {
						injectButtonsIntoControls($rowControls, parentModel, false);
					}
				}
			} else if (shortcode === 'vc_column_inner') {
				// For column_inner: .element-vc_column_inner .vc_advanced
				$controls = $el.find('.element-vc_column_inner .vc_advanced').first();
				// Also inject parent row_inner CS/PS into this column's row_inner controls
				var $rowInnerControls = $el.find('.parent-vc_row_inner .vc_advanced').first();
				if ($rowInnerControls.length) {
					var parentInnerId = model.get('parent_id');
					var parentInnerModel = parentInnerId && window.parent.vc.shortcodes ? window.parent.vc.shortcodes.get(parentInnerId) : null;
					if (parentInnerModel) {
						injectButtonsIntoControls($rowInnerControls, parentInnerModel, false);
					}
				}
			} else if (isSection) {
				// For sections: .vc_vc_section .element-vc_section
				$controls = $el.find('.vc_vc_section .element-vc_section').first();
			} else if (shortcode === 'vc_accordion') {
				// For accordion container: inject into ALL .parent-vc_accordion .vc_advanced
				$el.find('.parent-vc_accordion .vc_advanced').each(function() {
					injectButtonsIntoControls($(this), model, false);
				});
				return;
			} else if (shortcode === 'vc_accordion_tab') {
				// For accordion tab: .element-vc_accordion_tab .vc_advanced
				$controls = $el.find('.element-vc_accordion_tab .vc_advanced').first();
			} else if (shortcode === 'vc_tabs') {
				// For tabs container: inject into ALL .parent-vc_tabs .vc_advanced
				$el.find('.parent-vc_tabs .vc_advanced').each(function() {
					injectButtonsIntoControls($(this), model, false);
				});
				return;
			} else if (shortcode === 'vc_tab') {
				// For tab: .element-vc_tab .vc_advanced
				$controls = $el.find('.element-vc_tab .vc_advanced').first();
			} else if (shortcode === 'uncode_flexbox') {
				// For flexbox container: inject into ALL .parent-uncode_flexbox .vc_advanced
				$el.find('.parent-uncode_flexbox .vc_advanced').each(function() {
					injectButtonsIntoControls($(this), model, false);
				});
				return;
			} else if (shortcode === 'uncode_flexbox_inner') {
				// For flexbox inner: .element-uncode_flexbox_inner .vc_advanced
				$controls = $el.find('.element-uncode_flexbox_inner .vc_advanced').first();
				// Also inject parent flexbox CS/PS into this inner's flexbox controls
				var $flexboxControls = $el.find('.parent-uncode_flexbox .vc_advanced').first();
				if ($flexboxControls.length) {
					var flexParentId = model.get('parent_id');
					var flexParentModel = flexParentId && window.parent.vc.shortcodes ? window.parent.vc.shortcodes.get(flexParentId) : null;
					if (flexParentModel) {
						injectButtonsIntoControls($flexboxControls, flexParentModel, false);
					}
				}
			} else {
			...
			injectButtonsIntoControls($controls, model, false);
		}
